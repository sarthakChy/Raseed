...
function AnalysisResultCard({ analysisResult, onScanAnother, onAddToWallet }) {
    ...
    const handleAddToWallet = async () => {
        ...
        try {
            ...
            // Prepare the data for the wallet pass API
            const walletData = {
                store_name: receiptData.merchantName || "Unknown Store",
                total_amount: `₹${receiptData.total?.toFixed(2)}` || "₹0.00",
                date: receiptData.transactionDate 
                    ? new Date(receiptData.transactionDate).toLocaleDateString('en-US', { 
                        year: 'numeric', 
                        month: 'long', 
                        day: 'numeric' 
                    })
                ...
                category: receiptData.category || "Groceries",
    ...
    return (
        <div className="w-full max-w-md mx-auto font-sans bg-white rounded-2xl shadow-lg p-6">
            <div className="flex flex-col gap-6">
                ...
                {/* --- Google Wallet Link (Alternative) --- */}
                {walletResult && walletResult.wallet_link && (
                    <div className="text-center">
                        <a
                            href={walletResult.wallet_link}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-2 bg-white border-2 border-blue-600 text-blue-600 font-semibold py-2 px-4 rounded-lg hover:bg-blue-50 transition-colors"
                        >
                            <img 
                                src="https://developers.google.com/wallet/generic/resources/logo_googlewallet_48px.png" 
                                alt="Google Wallet" 
                                className="w-5 h-5"
                            />
