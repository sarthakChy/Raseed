...
//         const formData = new FormData();
//         formData.append('receipt_id', `"${analysisResult?.id}"`);
//         console.log(formData.get('receipt_id')); // Log the receipt ID for debugging

//         try {

//             const token = localStorage.getItem('Token');
//             const response = await fetch('http://localhost:8000/api/receipts/add', {
//                 method: 'POST',
//                 headers: { 'Authorization': `Bearer ${token}` },
//                 body: {receipt_id: formData.get('receipt_id'),},
//             });
        ...
        return (
            <div className="max-w-md mx-auto">
                ...
                <AnalysisResultCard 
                    analysisResult={analysisResult}
                    onScanAnother={handleScanAnother}
                    // Removed onAddToWallet prop since it's now handled internally
                />
            </div>
        );
    }
