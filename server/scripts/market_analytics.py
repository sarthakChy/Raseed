...
def get_master_item_analytics(client: bigquery.Client, table_id: str) -> list:
    """
    ...
    """
    return list(client.query(query.format(table_id=table_id)).result())

def get_peak_shopping_times(client: bigquery.Client, table_id: str) -> list:
    """
...
def get_average_repurchase_time(client: bigquery.Client, table_id: str) -> list:
    """
...
def get_analytics() -> str:
    ...
    table_id = f"{project_id}.raseed_analytics.receipts_raw_data_raw_changelog"
    ...
    try:
        ...
        item_stats = [convert_row_to_dict(row) for row in get_master_item_analytics(client, table_id)]
        print(f"Query 1 (Item Analytics) completed. {len(item_stats)} items found.")

        peak_times = [convert_row_to_dict(row) for row in get_peak_shopping_times(client, table_id)]
        print(f"Query 2 (Peak Shopping Times) completed. {len(peak_times)} records found.")

        repurchase_rates = [convert_row_to_dict(row) for row in get_average_repurchase_time(client, table_id)]
        print(f"Query 3 (Average Repurchase Time) completed. {len(repurchase_rates)} items found.")
        ...
        print("Successfully saved full analytics report to Firestore.")
        return "Pipeline completed successfully."

    except Exception as e:
        print(f"An error occurred in the analytics pipeline: {e}")
        # Re-raise the exception to see the full traceback if this is run from a larger script
        raise e

if __name__ == "__main__":
    # When you run this script directly, this block will execute.
    # Ensure GCP_PROJECT_ID environment variable is set before running.
    try:
